...
/// Compute the deviance −2 Σ_i log π(y_i | η*_i) for a single configuration.
///
/// `predictor` is the linear predictor η (length = n_obs), not necessarily the
/// full latent field when an A-matrix is used.
fn deviance_at_config(obs: &[Obs], predictor: &[f64]) -> Result<f64, String> {
    let n = obs.len();
    if predictor.len() != n {
        return Err("predictor length must match obs length".to_string());
    }
    let mut log_lik = 0.0;
    for i in 0..n {
        log_lik += eval_likelihood(predictor[i], &obs[i])?.logp;
    }
    Ok(-2.0 * log_lik)
}
...
/// - `cond_predictors`: for each integration point k, the conditional predictor η*(θ_k)
...
pub fn compute_dic(
    ...
    for k in 0..k_total {
        let d_k = deviance_at_config(obs, &cond_predictors[k])?;
        mean_deviance += norm_weights[k] * d_k;
    }

    let d_at_mode = deviance_at_config(obs, &cond_predictors[mode_index])?;
...
///
/// `cond_predictors` / `cond_pred_vars` are linear-predictor means/variances η
/// (length n_obs), not the full latent field when an A-matrix is used.
pub fn compute_cpo_pit(
    obs: &[Obs],
    cond_predictors: &[Vec<f64>],
    cond_pred_vars: &[Vec<f64>],
    norm_weights: &[f64],
    ...
    for k in 0..k_total {
        if cond_predictors[k].len() != n || cond_pred_vars[k].len() != n {
            return Err("each cond_predictors/cond_pred_vars entry must have length n".to_string());
        }
    }
    ...
    for i in 0..n {
        ...
        for k in 0..k_total {
            if check_cpo_failure_for_obs(&obs[i], cond_predictors[k][i], cond_pred_vars[k][i]) {
                failed = true;
                break;
            }
        }
        ...
        for k in 0..k_total {
            match eval_likelihood(cond_predictors[k][i], &obs[i]) {
        ...
        let mut pit_ok = true;
        for k in 0..k_total {
            match observation_cdf(&obs[i], cond_predictors[k][i], cond_pred_vars[k][i]) {
...
///
/// Returns `None` for families where the CDF is not yet implemented so that
/// PIT aggregation skips them without panicking.
fn observation_cdf(obs: &Obs, eta: f64, variance: f64) -> Option<f64> {
    match obs {
        ...
        // Explicitly unsupported for PIT (CPO still uses leave-one-out density paths elsewhere).
        Obs::NegativeBinomial(_)
        | Obs::ZeroInflatedPoisson(_)
        | Obs::ZeroInflatedBinomial(_)
        | Obs::Laplace(_)
...
mod tests {
    use super::*;
    use crate::inference::{
        ExponentialSurvivalObs, GaussianObs, LaplaceObs, Link, NegativeBinomialObs, Obs,
    ...
    #[test]
    fn pit_cdf_none_for_unimplemented_families() {
        // Families without CDF implementations must return None (no panic).
        let cases = [
        ...
        for obs in cases {
            assert!(observation_cdf(&obs, 0.0, 0.1).is_none());
        }
    }
    ...
    fn marginal_likelihoods_agree_roughly() {
        ...
        let build_prior = |theta: &[f64]| -> Result<inla_math::CscMatrix, String> {
            let tau = theta[0].exp();
