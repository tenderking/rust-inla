...
/// Build AR(1) precision as CSC (1-based triplet path via [`inla_math::triplets_to_csc`]).
pub fn ar1_precision_csc(n: usize, rho: f64, tau: f64) -> Result<inla_math::CscMatrix, String> {
    let q = ar1_precision(n, rho, tau)?;
    inla_math::triplets_to_csc(q.n, q.n, &q.i, &q.j, &q.x)
}
