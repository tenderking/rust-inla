...
pub fn laplace_newton_step(
    ...
    let n = q_prior.rows();
    let mut a = csc_to_dense(q_prior)?;
    for i in 0..n {
        a[i * n + i] += -evals[i].hess;
    }

    let factor = ldlt_factorize_dense(&a, n)?;
...
mod tests {
    use super::*;
    use crate::inference::Eval1D;
    use crate::sparse::ar1_precision_csc;

    #[test]
    fn factors_and_solves_ar1_matrix() {
        let csc = ar1_precision_csc(5, 0.5, 1.0).expect("csc");
    ...
    fn computes_laplace_newton_step() {
        let q = ar1_precision_csc(4, 0.3, 1.0).expect("q");
        let evals = vec![
            Eval1D {
                logp: 0.0,
                grad: 0.4,
    ...
    fn diagonal_inverse_matches_solves() {
        let csc = ar1_precision_csc(6, 0.4, 1.5).expect("csc");
        let f = ldlt_factorize(&csc).expect("factorize");
